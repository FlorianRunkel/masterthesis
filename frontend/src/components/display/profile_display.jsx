  ...
  return (
    <Box sx={{ bgcolor: '#fff', borderRadius: '16px', p: { xs: 2, sm: 3, md: 4 }, boxShadow: '0 2px 8px rgba(0,0,0,0.05)', mb: 4, width: '100%', mx: 'auto' }}>
      <Box className="profile-header" sx={{ display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, alignItems: { xs: 'flex-start', sm: 'center' }, justifyContent: 'space-between', gap: { xs: 2, sm: '20px' }, mb: { xs: 2, sm: '30px' } }}>
        ...
        {onSaveCandidate && (
          <Button variant="contained" startIcon={saving ? <CircularProgress size={10} color="inherit" /> : <SaveIcon />} onClick={onSaveCandidate} disabled={saving || saveSuccess} sx={{ bgcolor: '#001B41',color: '#fff',px: { xs: 1.5, sm: 2 },py: { xs: 1, sm: 1.2 },borderRadius: '8px','&:hover': { bgcolor: '#EB7836' },minWidth: { xs: '80px', sm: '100px' },fontSize: { xs: '0.8rem', sm: '0.8rem' },ml: { xs: 0, sm: 2 },mt: { xs: 0, sm: 2 }}}>{saving ? 'Save...' : 'Save candidate'}
          </Button>
        )}
      </Box>
        ...
        <Box>
          ...
          {profile.experience && profile.experience.length > 0 ? (
            profile.experience.map((exp, index) => (
              <Box key={index} sx={{ py: 1.5,  display: 'flex', gap: 2,position: 'relative','&::after': index < profile.experience.length - 1 ? { content: '""',position: 'absolute',left: '12px', top: '48px',bottom: '-16px',width: '2px', backgroundColor: '#e0e0e0'} : {}
              }}>
                <WorkIcon sx={{ color: '#EB7836', fontSize: { xs: '1.2rem', sm: '1.6rem' },mt: 0.5}} />
                <Box>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    ...
                    {exp.endDate === 'Present' && (
                      <Box sx={{display: 'inline-block',bgcolor: '#8AD265',color: '#fff',fontSize: '0.7rem',fontWeight: 600, px: 1.2, py: 0.2,borderRadius: 1, ml: 1, mb: 0.2
                      }}>
                        Current
                      </Box>
                    )}
        ...
        <Box>
          ...
          {profile.education && profile.education.length > 0 ? (
            profile.education.map((edu, idx) => (
              <Box key={idx} sx={{py: 1.5, display: 'flex',gap: 2, position: 'relative','&::after': idx < profile.education.length - 1 ? {content: '""',position: 'absolute',left: '12px',top: '48px',bottom: '-16px',width: '2px',backgroundColor: '#e0e0e0'} : {} }}>
                <SchoolIcon sx={{ color: '#EB7836', fontSize: { xs: '1.1rem', sm: '1.5rem' }, mt: 0.5 }} />
                <Box>
